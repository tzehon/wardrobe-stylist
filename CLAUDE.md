...
Phases 0, 1, 2a–2e, 3, 4, and 5 are done. Phase 5 shipped as a single Opus 4.8 `/recommend` call (no weather, no subagents in v1). Phase 6 is **in progress**: the on-device pieces are done — background receipt sync (`Wardrobe/Background/`, `BGProcessingTask`) and the daily-nudge notification (`Wardrobe/Notifications/`). What remains needs external accounts: **Fly.io/HTTPS deploy** (`backend/Dockerfile` + `backend/fly.toml` are committed and build-verified; run `fly deploy`, then flip `BACKEND_SCHEME=https` and remove the `NSAllowsArbitraryLoads` block from `Info.plist`) and **TestFlight** (Apple Developer Program). See `docs/setup.md`.
