...
Phases 0, 1, 2a–2e, 3, 4, and 5 are done. Phase 5 shipped as a single Opus 4.8 `/recommend` call (no weather, no subagents in v1). Phase 6 is **in progress**: the on-device pieces are done — background receipt sync (`Wardrobe/Background/`, `BGProcessingTask`) and the daily-nudge notification (`Wardrobe/Notifications/`). The backend is **deployed to Fly.io over HTTPS** (`backend/Dockerfile` + `backend/fly.toml`); the app points at it (`BACKEND_SCHEME=https`) and the dev-only ATS exception is gone. What remains is **TestFlight** distribution (Apple Developer Program) + the on-device camera QA. See `docs/setup.md`.
